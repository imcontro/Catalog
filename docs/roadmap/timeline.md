# Общая Хронология Работ

Этот файл хранит хронологическую историю завершенных work plans.

Новые записи добавляются сверху, чтобы последние изменения были видны первыми.

## Записи

### 2026-06-29 - Подготовка деплоя на Vercel

- Work plan: `docs/work-plans/completed/2026-06-29-vercel-deployment-prep.work-plan.md`
- Ветка: `feature/vercel-deployment-prep`
- Pull Request:
- Области: deployment, frontend, design, docs-process

Сделано:

- Specs обновлены под решение владельца: текущий чистый Supabase используется как рабочая база первой версии, перенос в отдельный Supabase-проект не требуется.
- В правилах деплоя зафиксировано, что `PUBLIC_SITE_URL` берется из Vercel-ссылки после создания проекта или из домена после подключения.
- Удалены устаревшие design artifacts админки `docs/design/admin-ui-map.html` и `docs/design/admin-paper-artboards.html`.
- Удален production-доступный черновой route `/design-draft/reference-layout`.
- Текущее состояние каталога подтверждено владельцем: `84` товара и `117` вкусов считаются правильными.

Проверено:

- `npm.cmd run lint`
- `npm.cmd run typecheck`
- `npm.cmd run build`
- `npm.cmd run db:check`
- `npm.cmd run storage:ensure`
- `npm.cmd run catalog:import:check` с ожидаемым расхождением старого контрольного импорта: `84` товара, `117` вкусов, `82/82` публичных ссылок фото открываются.
- `git diff --check`
- Локальная HTTP-проверка production server: `/`, `/cart`, `/admin/login`, `/api/catalog` вернули `HTTP 200`, `/design-draft/reference-layout` вернул `HTTP 404`.
- Владелец попросил сделать push.

Заметки:

- Домен не подключался.
- Vercel-проект, Pull Request и merge владелец делает сам.
- Схема базы данных, товары, цены, фото, категории и серверные API не менялись.

### 2026-06-28 - Проверка нестандартных состояний перед деплоем

- Work plan: `docs/work-plans/completed/2026-06-28-nonstandard-states-predeploy-check.work-plan.md`
- Ветка: `feature/nonstandard-states-predeploy-check`
- Pull Request:
- Области: catalog, cart-checkout-whatsapp, frontend

Сделано:

- Проверены нестандартные состояния клиентского каталога, корзины, оформления заказа и админки перед этапом деплоя.
- В specs и user stories обновлено правило вкусов: если клиент не выбрал вкус вручную, в корзину добавляется первый доступный вкус по порядку.
- В карточке товара недоступные вкусы остаются видимыми в списке и помечаются как **нет в наличии**, но не выбираются для заказа.
- В корзине текст бесплатной доставки приведен к общей формулировке specs: остаток до `8000` руб. или бесплатная доставка по г. Грозный.
- Подтверждено, что существующие админские ошибки входа, публикации незаполненного товара, удаления категории с товарами и загрузки неподходящего фото уже соответствуют specs.

Проверено:

- `npm.cmd run lint`
- `npm.cmd run typecheck`
- `npm.cmd run build`
- `git diff --check`
- HTTP 200 для `/`, `/cart` и `/admin/login`.
- Ручная проверка владельцем.
- Владелец попросил сделать push.

Заметки:

- Vercel, домен, Supabase, схема базы данных, серверные API и новые бизнес-функции не менялись.
- Pull Request и merge владелец делает сам на GitHub.

### 2026-06-28 - UI/UX редизайн админки

- Work plan: `docs/work-plans/completed/2026-06-28-admin-ui-ux-redesign.work-plan.md`
- Ветка: `feature/admin-ui-ux-redesign`
- Pull Request:
- Области: admin, frontend, design

Сделано:

- Утвержденная графическая карта админки перенесена в рабочие страницы `/admin`.
- Desktop-админка приведена к направлению `docs/design/admin-paper-artboards.html`: рабочий board шириной 1080px, верхняя шапка, левое меню, карточки товаров, toolbar с поиском и действиями.
- Телефонная версия админки использует компактные списки, горизонтальную навигацию и крупные основные действия.
- Обновлены `/admin/login`, `/admin`, `/admin/products`, `/admin/drafts`, `/admin/hidden`, `/admin/products/new`, `/admin/products/[id]/edit`, `/admin/categories`, `/admin/sorting/all` и `/admin/sorting/category/[id]`.
- Форма товара сгруппирована по смыслу: основные данные, цена и упаковка, статус, фото и вкусы.
- Статусы, опасные действия, пустые состояния и ошибочные состояния приведены к единому визуальному стилю.
- Бизнес-логика, API, схема базы данных, Supabase Storage, клиентский каталог, корзина и WhatsApp-сообщение не менялись.

Проверено:

- `npm.cmd run lint`
- `npm.cmd run typecheck`
- `npm.cmd run build`
- `git diff --check`
- Ручная проверка владельцем.
- Владелец утвердил дизайн и попросил сделать push.

Заметки:

- UI админки перенесен по утвержденному design artifact без добавления новых функций управления каталогом.
- Pull Request и merge владелец делает сам на GitHub.

### 2026-06-28 - Клиентский каталог: цена за штуку и desktop-прокрутка категорий

- Work plan: `docs/work-plans/completed/2026-06-28-client-catalog-unit-price-and-desktop-categories.work-plan.md`
- Ветка: `feature/client-catalog-unit-price-and-desktop-categories`
- Pull Request:
- Области: catalog, frontend

Сделано:

- В карточку товара добавлена примерная цена за штуку в формате `≈ 27,5 ₽/шт`.
- Цена за штуку вычисляется автоматически от текущей эффективной цены карточки и количества штук в упаковке.
- При выборе вкуса с отдельной ценой цена за штуку пересчитывается от цены выбранного вкуса.
- В клиентский DTO добавлены `unitPriceRub` и `unitPriceLabel` без изменения схемы базы данных.
- Для desktop-версии горизонтального списка категорий добавлены кнопки прокрутки влево и вправо.
- Мобильное поведение категорий со свайпом сохранено.
- Specs клиентского каталога, technical spec и design spec обновлены под цену за штуку.

Проверено:

- `npm.cmd run lint`
- `npm.cmd run typecheck`
- `npm.cmd run build`
- `git diff --check`
- Ручная проверка владельцем.
- Владелец попросил сделать push.

Заметки:

- Схема базы данных, миграции, импорт, админка, корзина, WhatsApp-сообщение и деплой на Vercel не менялись.
- Pull Request и merge владелец делает сам на GitHub.

### 2026-06-28 - Админка: ручная сортировка каталога

- Work plan: `docs/work-plans/completed/2026-06-28-admin-manual-sorting.work-plan.md`
- Ветка: `feature/admin-manual-sorting`
- Pull Request:
- Области: admin, frontend

Сделано:

- На странице `/admin/categories` добавлена ручная сортировка реальных категорий.
- Добавлен защищенный раздел `/admin/sorting/all` для ручной сортировки товаров в фильтре **Все напитки**.
- Добавлен защищенный раздел `/admin/sorting/category/{id}` для ручной сортировки товаров внутри отдельной категории.
- Добавлены защищенные API `POST /api/admin/categories/reorder`, `POST /api/admin/products/reorder-all` и `POST /api/admin/categories/{id}/products/reorder`.
- Сервер проверяет админскую сессию, полный список `id`, неизвестные или повторяющиеся элементы и принадлежность товаров выбранной категории.
- Сервер сохраняет порядок в `categories.sort_order`, `products.all_drinks_sort_order` и `products.category_sort_order` с нормализацией без пустых мест.
- Интерфейс сортировки поддерживает перетаскивание и кнопки **Вверх** / **Вниз** для удобной проверки на телефоне.
- Порядок **Все напитки** и порядок внутри категории остаются независимыми.

Проверено:

- `npm.cmd run lint`
- `npm.cmd run typecheck`
- `npm.cmd run build`
- `git diff --check`
- `/admin/sorting/all` и `/admin/sorting/category/{id}` без сессии переводят на `/admin/login`.
- API сортировки без сессии возвращают `401`.
- Ручная проверка владельцем.
- Владелец попросил сделать push.

Заметки:

- Перенос товаров между категориями, редактирование товаров, сортировка вкусов и изменение клиентского дизайна не входили в этот work plan.
- Pull Request и merge владелец делает сам на GitHub.

### 2026-06-28 - Админка: управление категориями

- Work plan: `docs/work-plans/completed/2026-06-27-admin-categories-management.work-plan.md`
- Ветка: `feature/admin-categories-management`
- Pull Request:
- Области: admin, frontend

Сделано:

- Добавлен защищенный раздел `/admin/categories` для управления реальными категориями из таблицы `categories`.
- Владелец может добавить новую категорию, переименовать существующую категорию и удалить пустую категорию.
- Фильтр **Все напитки** оставлен системным: он не создается как запись в базе и не редактируется в списке категорий.
- Добавлены защищенные API `GET /api/admin/categories`, `POST /api/admin/categories`, `PATCH /api/admin/categories/{id}` и `DELETE /api/admin/categories/{id}`.
- Сервер проверяет админскую сессию, непустое название, уникальность названия, существование категории и наличие товаров перед удалением.
- Категория с неудаленными товарами не удаляется, а владелец видит понятное сообщение о переносе или удалении товаров.
- Уже удаленные товары отвязываются от категории перед удалением пустой категории, чтобы мягко удаленные записи не блокировали удаление.
- Ссылка на раздел категорий добавлена на главную админки и в навигацию списков товаров.
- Интерфейс управления категориями адаптирован под телефон и desktop.

Проверено:

- `npm.cmd run lint`
- `npm.cmd run typecheck`
- `npm.cmd run build`
- `git diff --check`
- `/admin/categories` без сессии переводит на `/admin/login`.
- Ручная проверка владельцем.
- Владелец попросил сделать push.

Заметки:

- Drag-and-drop сортировка категорий и массовый перенос товаров между категориями не входили в этот work plan.
- Pull Request и merge владелец делает сам на GitHub.

### 2026-06-27 - Админка: быстрая отметка товара нет в наличии

- Work plan: `docs/work-plans/completed/2026-06-27-admin-product-stock-quick-action.work-plan.md`
- Ветка: `feature/admin-product-stock-quick-action`
- Pull Request:
- Области: admin, frontend

Сделано:

- На карточку активного товара в `/admin/products` добавлено быстрое действие **Нет в наличии**.
- На карточку товара со статусом `out_of_stock` добавлено быстрое действие **В наличие**.
- Перед сменой статуса показывается подтверждение.
- Добавлены защищенные API `POST /api/admin/products/{id}/mark-out-of-stock` и `POST /api/admin/products/{id}/mark-active`.
- Сервер проверяет админскую сессию, существование товара, отсутствие удаления и допустимость смены статуса.
- Перед возвратом товара в `active` сервер проверяет обязательные данные для публикации.
- После успешного действия список админки обновляется через существующий механизм `router.refresh()`.
- Клиентский каталог, корзина и WhatsApp продолжают работать по существующим правилам доступности товара.

Проверено:

- `npm.cmd run typecheck`
- `npm.cmd run lint`
- `npm.cmd run build`
- `git diff --check`
- Ручная проверка владельцем.
- Владелец попросил сделать push.

Заметки:

- Форма редактирования товара, вкусы, скрытие, удаление, категории, сортировка и клиентский интерфейс не менялись.
- Pull Request и merge владелец делает сам на GitHub.

### 2026-06-27 - Админка: управление вкусами товара

- Work plan: `docs/work-plans/completed/2026-06-26-admin-product-flavors.work-plan.md`
- Ветка: `feature/admin-product-flavors`
- Pull Request:
- Области: admin, frontend

Сделано:

- В форму создания и редактирования товара добавлено управление признаком **Выбор вкуса**.
- Владелец может добавлять, редактировать и удалять вкусы товара в существующей форме товара.
- Для каждого вкуса можно указать название, отдельную цену, фото вкуса и отметку **Нет в наличии**.
- Фото вкуса загружается через существующий защищенный API `POST /api/admin/images`.
- При редактировании товара форма читает существующие вкусы в порядке `product_flavors.sort_order`.
- `POST /api/admin/products` и `PATCH /api/admin/products/{id}` принимают `hasFlavorChoice` и список вкусов.
- Сервер сохраняет товар и вкусы в одной транзакции, проверяет пустые названия вкусов, некорректные цены и ссылки на фото.
- При выключенном выборе вкуса товар сохраняется без записей вкусов для клиентского сценария.
- Клиентский каталог, корзина и WhatsApp продолжают использовать существующие правила эффективной цены, фото и доступности вкуса.

Проверено:

- `npm.cmd run typecheck`
- `npm.cmd run lint`
- `npm.cmd run build`
- `git diff --check`
- Ручная проверка владельцем.
- Владелец попросил сделать push.

Заметки:

- Отдельный раздел админки для вкусов, drag-and-drop сортировка вкусов, очистка старых фото и деплой на Vercel не входили в этот work plan.
- Pull Request и merge владелец делает сам на GitHub.

### 2026-06-26 - Админка: быстрые действия и удаление товара

- Work plan: `docs/work-plans/completed/2026-06-26-admin-product-quick-actions-delete.work-plan.md`
- Ветка: `feature/admin-product-quick-actions-delete`
- Pull Request:
- Области: admin, frontend

Сделано:

- На карточках товаров в `/admin/products` добавлено быстрое действие **Скрыть** с подтверждением.
- На карточках товаров в `/admin/hidden` добавлено действие **Вернуть в каталог**.
- Возврат скрытого товара публикует его в статус `active` только при наличии обязательных данных.
- На карточках товаров в `/admin/products`, `/admin/drafts` и `/admin/hidden` добавлено действие **Удалить** с подтверждением.
- Добавлен защищенный `DELETE /api/admin/products/{id}` с мягким удалением через `products.deleted_at`.
- Добавлены защищенные API `POST /api/admin/products/{id}/hide` и `POST /api/admin/products/{id}/restore`.
- Удаленные товары исчезают из списков админки и не попадают в клиентский каталог.
- Фото товара не удаляются из Supabase Storage при удалении товара.

Проверено:

- `npm.cmd run lint`
- `npm.cmd run typecheck`
- `npm.cmd run build`
- `git diff --check`
- Владелец попросил сделать push.

Заметки:

- Управление вкусами, категориями, сортировкой, фото вкусов и восстановление удаленного товара не входили в этот work plan.
- Временные логи `.codex_tmp/` не входят в commit.

### 2026-06-26 - Админка: загрузка и замена основного фото товара

- Work plan: `docs/work-plans/completed/2026-06-26-admin-product-main-image.work-plan.md`
- Ветка: `feature/admin-product-main-image`
- Pull Request:
- Области: admin, frontend

Сделано:

- Добавлен защищенный API `POST /api/admin/images` для загрузки фото после входа в админку.
- Фото проверяется по формату JPG, PNG или WebP и лимиту до 5 МБ.
- Файл сохраняется в Supabase Storage bucket `product-images`, а данные фото записываются в таблицу `images`.
- Форма создания и редактирования товара позволяет загрузить или заменить основное фото.
- При сохранении товара форма передает `mainImageId`, а сервер записывает его в `products.main_image_id`.
- Правило обязательного основного фото для публикации товара сохранено.

Проверено:

- `npm.cmd run lint`
- `npm.cmd run typecheck`
- `npm.cmd run build`
- `git diff --check`
- Временная HTTP-проверка `/admin/login`: `HTTP 200`
- Ручная проверка владельцем.
- Владелец попросил сделать push.

Заметки:

- Фото вкусов, управление вкусами, удаление старых фото и очистка неиспользуемых фото не входили в этот work plan.
- Временные логи `.codex_tmp/` не входят в commit.

### 2026-06-26 - Админка: добавление и редактирование базовых данных товаров

- Work plan: `docs/work-plans/completed/2026-06-25-admin-product-basic-editing.work-plan.md`
- Ветка: `feature/admin-product-basic-editing`
- Pull Request:
- Области: admin, frontend

Сделано:

- Добавлены защищенные страницы `/admin/products/new` и `/admin/products/[id]/edit`.
- В админке появился первый сценарий создания товара как черновика.
- Добавлено редактирование базовых данных товара: название, категория, цена, количество штук в одной уп и статус.
- Добавлены защищенные API `POST /api/admin/products` и `PATCH /api/admin/products/{id}`.
- Серверная проверка не публикует товар со статусом `active` или `out_of_stock`, если не заполнены обязательные данные для клиентского каталога.
- При смене категории товар переносится в конец новой категории, а старая категория смыкается без пустого места.
- На карточках товаров в `/admin/products`, `/admin/drafts` и `/admin/hidden` добавлено действие **Редактировать**.

Проверено:

- `npm.cmd run lint`
- `npm.cmd run typecheck`
- `npm.cmd run build`
- `git diff --check`
- Владелец попросил сделать push.

Заметки:

- Загрузка и замена фото, управление вкусами, удаление товаров, управление категориями и drag-and-drop сортировка не входили в этот work plan.
- Временные логи `.codex_tmp/` не входят в commit.

### 2026-06-25 - Админка: карточки товаров по категориям

- Work plan: `docs/work-plans/completed/2026-06-25-admin-category-card-overview.work-plan.md`
- Ветка: `feature/admin-category-card-overview`
- Pull Request:
- Области: admin, frontend

Сделано:

- `/admin/products` переведен со строкового списка на карточный обзор товаров.
- Добавлены переключатели категорий: **Все напитки** первым, затем реальные категории в порядке `categories.sort_order`.
- В режиме **Все напитки** товары сортируются по `all_drinks_sort_order`, внутри категории - по `category_sort_order`.
- Поиск по названию сохранен и работает вместе с выбранной категорией.
- `/admin/drafts` и `/admin/hidden` используют тот же карточный вид товара.
- Добавлен мобильный адаптив админских карточек, вкладок, категорий, поиска и шапки.

Проверено:

- `npm.cmd run lint`
- `npm.cmd run typecheck`
- `npm.cmd run build`
- `git diff --check`
- Ручная проверка владельцем, включая мобильный вид.
- Владелец попросил сделать push.

Заметки:

- Добавление, редактирование, удаление, изменение фото, цены, статусов, вкусов, категорий и drag-and-drop сортировка не входили в этот work plan.

### 2026-06-25 - Админка: список товаров и поиск

- Work plan: `docs/work-plans/completed/2026-06-24-admin-products-list.work-plan.md`
- Ветка: `feature/admin-products-list`
- Pull Request:
- Области: admin, frontend

Сделано:

- Обновлена закрытая страница `/admin`: добавлены входы в разделы **Товары**, **Черновики** и **Скрытые**.
- Добавлены защищенные страницы `/admin/products`, `/admin/drafts` и `/admin/hidden`.
- Обычный список товаров показывает только неудаленные товары со статусами `active` и `out_of_stock`.
- Черновики и скрытые товары вынесены в отдельные защищенные списки.
- Добавлен серверный поиск товаров по названию на странице `/admin/products`.
- В строке товара показываются фото, название, категория, цена, количество штук в одной уп, статус и признак наличия вкусов.
- Добавлены русские пустые состояния для списка, поиска, черновиков и скрытых товаров.

Проверено:

- `npm.cmd run lint`
- `npm.cmd run typecheck`
- `npm.cmd run build`
- `git diff --check`
- Владелец попросил сделать push.

Заметки:

- Добавление, редактирование, удаление, скрытие, возврат товаров, управление фото, вкусами, категориями и сортировкой не входили в этот work plan.
- Dev-сервер напрямую стартовал командой `npm.cmd run dev -- --hostname 127.0.0.1 --port 3001`.

### 2026-06-24 - Админка: вход

- Work plan: `docs/work-plans/completed/2026-06-24-admin-auth.work-plan.md`
- Ветка: `feature/admin-auth`
- Pull Request:
- Области: admin

Сделано:

- Подключена рабочая форма входа в админку `/admin/login`.
- Добавлены API `POST /api/admin/login` и `POST /api/admin/logout`.
- Реализована серверная админская сессия через `httpOnly` cookie.
- В таблице `admin_sessions` хранится хеш токена сессии, а не сам токен.
- Закрытая страница `/admin` проверяет сессию на сервере и без входа переводит на `/admin/login`.
- Добавлена кнопка выхода из админки.
- В technical spec зафиксирован формат `ADMIN_PASSWORD_HASH`.

Проверено:

- `npm.cmd run lint`
- `npm.cmd run typecheck`
- `npm.cmd run build`
- `npm.cmd run db:check`
- `git diff --check`
- Ручной сценарий входа, защиты `/admin`, выхода и повторного открытия `/admin` после выхода.
- Владелец попросил сделать push.

Заметки:

- Управление товарами, категориями, вкусами, фото и сортировкой не входило в этот work plan.
- Тестовые локальные значения `.env` не входят в Git.

### 2026-06-24 - Формат WhatsApp-сообщения без эмодзи с жирными подписями

- Work plan: `docs/work-plans/completed/2026-06-18-whatsapp-message-format.work-plan.md`
- Ветка: `feature/whatsapp-message-format`
- Pull Request:
- Области: cart-checkout-whatsapp

Сделано:

- Обновлен формат WhatsApp-сообщения заказа: постоянные подписи выделены жирным через WhatsApp-разметку `*...*`.
- Приветствие, блок товаров, сумма, доставка, адрес и оплата оформлены без эмодзи и декоративных маркеров.
- Строки товаров оставлены компактными в формате `Название товара вкус - 1уп`.
- Значения заказа оставлены обычным текстом.
- Значение доставки в WhatsApp-сообщении больше не дублирует подпись `Доставка:`.
- Обновлены связанные feature spec, technical spec и user story.

Проверено:

- `npm.cmd run lint`
- `npm.cmd run typecheck`
- `npm.cmd run build`
- `git diff --check`
- Владелец попросил сделать push.

Заметки:

- Бизнес-логика корзины, доставки, актуализации позиций и очистки корзины не менялась.
- Поля оформления заказа, номер WhatsApp, Supabase, админка, миграции и API не менялись.

### 2026-06-18 - Редизайн клиентского интерфейса

- Work plans:
  - `docs/work-plans/completed/2026-06-17-client-interface-paper-design.work-plan.md`
  - `docs/work-plans/completed/2026-06-17-reference-layout-draft.work-plan.md`
  - `docs/work-plans/completed/2026-06-18-client-interface-reference-layout-transfer.work-plan.md`
- Ветка: `feature/client-interface-redesign`
- Pull Request:
- Области: catalog, cart-checkout-whatsapp, frontend, design

Сделано:

- Создана черновая страница `/design-draft/reference-layout` для визуального утверждения нового клиентского интерфейса.
- Утвержденный reference layout перенесен в основной каталог `/`, корзину `/cart` и окно оформления заказа.
- Обновлены карточки товаров, поиск, категории, нижняя кнопка корзины, страница корзины и оформление заказа.
- Сохранена текущая бизнес-логика каталога, корзины, доставки и WhatsApp-сообщения.
- Обновлены связанные specs и user story под утвержденный визуальный подход.

Проверено:

- `npm.cmd run lint`
- `npm.cmd run typecheck`
- `npm.cmd run build`
- `git diff --check`
- Владелец проверил утвержденный дизайн и попросил сделать push.

Заметки:

- Админка, Supabase, миграции, формат WhatsApp-сообщения и бизнес-логика заказа не менялись.

### 2026-06-14 - Страница корзины и окно оформления заказа

- Work plan: `docs/work-plans/completed/2026-06-14-cart-page-checkout-modal.work-plan.md`
- Ветка: `feature/cart-page-checkout-modal`
- Pull Request:
- Области: cart-checkout-whatsapp, frontend, catalog, design

Сделано:

- Добавлена отдельная публичная страница корзины `/cart`.
- Полный состав корзины перенесен из окна на страницу корзины с фото, названием, выбранным вкусом, ценой, количеством и суммой позиции.
- Фото позиции берется из выбранного вкуса, если оно есть, иначе используется основное фото товара.
- В каталоге компактная кнопка корзины ведет на `/cart` и остается видимой даже при пустой корзине.
- После добавления товара в каталоге показывается дополнительная кнопка **Корзина** со счетчиком уп.
- Оформление заказа перенесено из отдельной страницы в почти полноэкранное окно поверх `/cart`.
- Старый маршрут `/checkout` удален без редиректа.
- Корзина актуализируется через `POST /api/cart/resolve`, скрытые и удаленные позиции удаляются, недоступные остаются видимыми и не входят в сумму.
- После открытия WhatsApp-ссылки корзина очищается.
- Обновлены feature specs, technical specs и user story под путь `Каталог -> /cart -> окно оформления -> WhatsApp`.

Проверено:

- `npm.cmd run lint`
- `npm.cmd run typecheck`
- `npm.cmd run build`
- `git diff --check`
- Владелец проверил локальный сценарий корзины, оформления заказа и попросил сделать push.

Заметки:

- Заказ не сохраняется в базе данных.
- Онлайн-оплата, админка, деплой на Vercel и изменение схемы базы данных не входили в этот work plan.

### 2026-06-12 - Оформление заказа и WhatsApp

- Work plan: `docs/work-plans/completed/2026-06-12-checkout-whatsapp.work-plan.md`
- Ветка: `feature/checkout-whatsapp`
- Pull Request:
- Области: cart-checkout-whatsapp, frontend, design

Сделано:

- Добавлена страница оформления заказа `/checkout`.
- Из корзины добавлен переход **Оформить заказ**, доступный только при наличии доступных позиций.
- На оформлении заказа показываются выбранные позиции, недоступные позиции, сумма доступных товаров и итоговая строка доставки.
- Корзина актуализируется через `POST /api/cart/resolve` при открытии оформления и повторно перед отправкой в WhatsApp.
- Добавлены обязательный адрес доставки с примером `Грозный ул. Назабраева 92` и обязательный выбор оплаты: наличные или перевод.
- Реализовано формирование WhatsApp-сообщения с приветствием, компактным списком товаров, суммой, доставкой, адресом и оплатой.
- В WhatsApp-сообщение попадают только доступные позиции, а недоступные позиции остаются видимыми на оформлении и не участвуют в сумме.
- После открытия WhatsApp корзина очищается.
- Формат WhatsApp-сообщения оставлен без эмодзи, чтобы на устройствах клиента и работника не появлялись вопросительные знаки.
- Обновлены specs, technical spec, user story и active/completed work plan под финальный формат сообщения.

Проверено:

- `npm.cmd run lint`
- `npm.cmd run typecheck`
- `npm.cmd run build`
- `git diff --check`
- HTTP 200 для `/` и `/checkout`
- Владелец проверил локальное оформление заказа, WhatsApp-сообщение и попросил сделать push.

Заметки:

- Заказ не сохраняется в базе данных.
- Онлайн-оплата, админка и деплой на Vercel не входили в этот work plan.
- Полный редизайн клиентского интерфейса остается отдельным будущим work plan.

### 2026-06-12 - Корзина клиента

- Work plan: `docs/work-plans/completed/2026-06-11-cart.work-plan.md`
- Ветка: `feature/cart`
- Pull Request:
- Области: cart-checkout-whatsapp, frontend, catalog, design

Сделано:

- Добавлено добавление доступных товаров и выбранных вкусов в корзину из клиентского каталога.
- Корзина хранится на клиенте в `localStorage` с ключом `napitki_berkat_cart` и сохраняется после обновления страницы.
- На карточке товара после добавления показывается управление количеством: минус, количество `уп` и плюс.
- Добавлена компактная кнопка открытия корзины с иконкой, текстом **Корзина** и счетчиком выбранных уп.
- Корзина открывается отдельным окном, а на телефоне - нижней панелью с крупными кнопками управления.
- В корзине показываются позиции, выбранные вкусы, цена, количество, сумма позиции и общая сумма товаров.
- Добавлены удаление позиции, очистка всей корзины и автоматическое закрытие корзины после удаления последней позиции.
- Добавлен серверный `POST /api/cart/resolve` для актуализации корзины по данным Supabase.
- Скрытые и удаленные позиции удаляются из корзины при актуализации, недоступные позиции остаются видимыми, но не участвуют в сумме.
- Добавлена подсказка по бесплатной доставке по г. Грозный от суммы `8000` руб.
- В design spec зафиксировано направление будущего визуального оформления по примерам владельца для телефона и компьютера.

Проверено:

- `npm.cmd run lint`
- `npm.cmd run typecheck`
- `npm.cmd run build`
- `git diff --check`
- Владелец проверил локальную корзину и попросил сделать push.

Заметки:

- Этот план не включает оформление заказа, ввод адреса, выбор оплаты, WhatsApp-сообщение и очистку корзины после WhatsApp.
- Полный редизайн клиентского интерфейса по предоставленным примерам остается отдельным следующим work plan.

### 2026-06-06 - Клиентский каталог на данных Supabase

- Work plan: `docs/work-plans/completed/2026-06-06-client-catalog.work-plan.md`
- Ветка: `feature/client-catalog`
- Pull Request:
- Области: catalog, frontend, design

Сделано:

- Главная страница `/` показывает клиентский каталог на реальных данных тестового Supabase.
- Добавлен серверный `GET /api/catalog` и серверная сборка DTO каталога без служебных ключей и секретов.
- Категории, товары, вкусы, цены, фото, статусы и сортировка берутся из таблиц `categories`, `products`, `product_flavors` и `images`.
- Клиентский каталог показывает только видимые товары и не показывает черновики, скрытые и удаленные товары.
- Добавлены фильтр **Все напитки**, фильтры категорий, поиск по названию товара и выбор вкусов.
- При выборе вкуса карточка меняет цену и фото, если такие данные есть, а выбранный вкус показывается в названии цветной вставкой.
- Карточки товаров адаптированы под мобильную ширину, фото ограничены рамкой и не растягивают карточку.
- Названия категорий, товаров и вкусов отображаются с заглавной буквы.

Проверено:

- `npm.cmd run lint`
- `npm.cmd run typecheck`
- `npm.cmd run build`
- `npm.cmd run db:check`
- `npm.cmd run catalog:import:check`
- `git diff --check`
- Владелец проверил локальный каталог на `http://127.0.0.1:3000` и попросил сделать push.

Заметки:

- `npm.cmd run catalog:import:check` подтвердил 7 категорий, 86 товаров, 108 вкусов, 82 images и 82 файла в Storage.
- Публичные ссылки фото открываются для 82 из 82 импортированных фото.
- Изменения не включают корзину, оформление заказа, WhatsApp, вход в админку и деплой на Vercel.

### 2026-06-06 - Загрузка данных старого каталога в Supabase

- Work plan: `docs/work-plans/completed/2026-06-06-catalog-data-import.work-plan.md`
- Ветка: `feature/catalog-data-import`
- Pull Request:
- Области: data-supabase

Сделано:

- В подтвержденный тестовый Supabase загружены реальные данные старого каталога.
- В Supabase Postgres записаны 86 товаров и 108 вкусов.
- В Supabase Storage bucket `product-images` загружены 82 фото в папку `old-catalog/`.
- Добавлена команда проверки результата импорта `npm.cmd run catalog:import:check`.
- Подготовлен отчет `data/import/old-catalog-import-result-report.md`.
- Обновлен preparation-report после повторного dry-run: повторный запуск обновляет существующие записи, а не создает дубли.

Проверено:

- `npm.cmd run lint`
- `npm.cmd run typecheck`
- `npm.cmd run build`
- `npm.cmd run storage:ensure`
- `npm.cmd run catalog:import:validate`
- `npm.cmd run catalog:import:dry-run`
- `npm.cmd run db:check`
- `npm.cmd run catalog:import:check`
- `git diff --check`
- Владелец проверил результат и попросил сделать push.

Заметки:

- `npm.cmd run catalog:import:check` подтвердил 7 категорий, 86 товаров, 108 вкусов, 82 images и 82 файла в Storage.
- Публичные ссылки открываются для 82 из 82 импортированных фото.
- 4 товара остались черновиками из-за отсутствующего основного фото: `горная 5л`, `К-13`, `родники 0,5л`, `южания 0.2 детский`.
- Боевой Supabase не использовался в рамках этого work plan.

### 2026-06-06 - Подготовка импорта старого каталога

- Work plan: `docs/work-plans/completed/2026-06-06-catalog-import-preparation.work-plan.md`
- Ветка: `feature/catalog-import-preparation`
- Pull Request:
- Области: data-supabase

Сделано:

- Добавлена валидация draft-данных старого каталога.
- Добавлен dry-run импорта без записи в Supabase.
- Подготовлен маппинг категорий старого каталога на категории проекта.
- Добавлена проверка фото, черновиков, дублей товаров и дублей вкусов.
- Подготовлен отчет `data/import/old-catalog-import-preparation-report.md`.
- Реальная запись в Supabase защищена отдельным confirm-флагом и не запускалась в рамках этого plan.

Проверено:

- `npm.cmd run lint`
- `npm.cmd run typecheck`
- `npm.cmd run build`
- `npm.cmd run catalog:import:validate`
- `npm.cmd run catalog:import:dry-run`
- `git diff --check`
- Владелец проверил результат и попросил сделать push.

Заметки:

- Dry-run показал 7 найденных категорий, 82 images к созданию, 86 товаров к созданию и 108 вкусов к созданию.
- 82 товара готовы к клиентскому каталогу, 4 товара останутся черновиками из-за отсутствующего основного фото.
- Финальная загрузка реальных товаров и фото в Supabase остается отдельным следующим work plan.

### 2026-06-06 - Supabase и база данных

- Work plan: `docs/work-plans/completed/2026-06-06-supabase-database.work-plan.md`
- Ветка: `feature/supabase-database`
- Pull Request:
- Области: data-supabase

Сделано:

- Выбран и зафиксирован Drizzle ORM для работы с Supabase Postgres.
- Добавлены зависимости, конфигурация Drizzle и серверные модули подключения к базе.
- Подготовлены схема и миграция для таблиц `categories`, `products`, `product_flavors`, `images` и `admin_sessions`.
- Добавлены стартовые категории каталога.
- Добавлена серверная проверка чтения из базы.
- Подготовлен Supabase Storage bucket `product-images`.
- Обновлен `.env.example` без реальных секретов.

Проверено:

- `npm.cmd run lint`
- `npm.cmd run typecheck`
- `npm.cmd run build`
- `npm.cmd run db:migrate`
- `npm.cmd run db:check`
- `npm.cmd run storage:ensure`
- `git diff --check`
- Проверено, что локальный `.env` игнорируется Git.

Заметки:

- Реальные товары и фото старого каталога не загружались.
- Клиентский каталог, корзина, WhatsApp-заказ и админка управления товарами не входили в этот work plan.
- Pull Request и merge владелец делает сам на GitHub.

### 2026-06-05 - Подготовка приложения

- Work plan: `docs/work-plans/completed/2026-06-05-application-foundation.work-plan.md`
- Ветка: `feature/application-foundation`
- Pull Request:
- Области: frontend, design, admin, catalog, docs-process

Сделано:

- Создана основа Next.js / React / TypeScript приложения.
- Добавлены базовые страницы `/` и `/admin/login`.
- Добавлены общие стили, использование логотипа и русские тексты интерфейса.
- Добавлены `.env.example`, `.gitignore`, TypeScript, ESLint и Next.js конфиги.
- Настроен `npm run dev` через Turbopack, чтобы локальный dev-сервер стабильно запускался на этой машине.

Проверено:

- Владелец открыл сайт в браузере и подтвердил, что все нормально.
- `npm run lint`
- `npm run typecheck`
- `npm run build`
- `npm audit --audit-level=high`
- HTTP 200 для `/` и `/admin/login`.
- `git diff --check`

Заметки:

- Supabase, реальные товары, корзина, WhatsApp и настоящая авторизация админки не входят в этот work plan.

## Шаблон записи

```text
### YYYY-MM-DD - Название work plan

- Work plan: `docs/work-plans/completed/file-name.work-plan.md`
- Ветка: `branch-name`
- Pull Request: `PR link or empty`
- Области: frontend, design, admin

Сделано:

- ...

Проверено:

- ...

Заметки:

- ...
```
